...
def calibration():
    ...
    ### capturing eye and calibration start
    ...
    ### finding eye movement window

    xs = []
    ys = []

    for pos in eyePoses.values():
        xs.append(pos[0])
        ys.append(pos[1])

    xmin = xs[np.argmin(xs)]
    xmax = xs[np.argmax(xs)]
    ymin = ys[np.argmin(ys)]
    ymax = ys[np.argmax(ys)]
